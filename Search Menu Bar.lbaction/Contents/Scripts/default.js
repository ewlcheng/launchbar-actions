...
function run(argument) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (mainMenuBarItemName != 'Apple') {
      const menuItemName = line.substring(
        'menu item '.length,
        line.indexOf(' of menu ')
      );
      ...
      if (middleLevels.indexOf(' of menu item ') < 0) {
        ...
        while (index >= 0) {
          const menuItem = {};
          menuItem[middleLevelsLine.substring(startIndex, index).trim()] =
            previousMenuItem;

          middleLevelsLine = middleLevelsLine.substring(
            index + ' of menu item '.length
          );
          startIndex =
            middleLevelsLine.indexOf(' of menu ') + ' of menu '.length;
